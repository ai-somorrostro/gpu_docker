#!/bin/bash
# Script para entrenar modelos RT-DETR
...
if [ "$1" == "--help" ] || [ "$1" == "-h" ]; then
    ...
    echo ""
    echo "Argumentos:"
    echo "  script       - Script Python a ejecutar (default: scripts/train_rtdetr.py)"
    ...
    echo "  DOCKER_IMAGE - Nombre de la imagen Docker (default: rtdetr-training-image)"
    echo ""
    echo "Ejemplos:"
    echo "  $0"
    echo "  $0 --detach train_rtdetr.py datasets/coco8/coco8 config/coco8.yaml"
    echo "  $0 -d train_rtdetr.py datasets/coco8/coco8 config/coco8.yaml"
    echo ""
    exit 0
fi

# Parámetros
SCRIPT="${1:-scripts/train_rtdetr.py}"
DATASET_DIR="${2:-datasets}"
DATASET_YAML="${3:-}"
ENV_FILE="${4:-.env}"
DOCKER_IMAGE="${DOCKER_IMAGE:-rtdetr-training-image}"
...
# Caché compartida para modelos RT-DETR
...
if [ "$DETACH_MODE" = true ]; then
    CONTAINER_NAME="rtdetr-train-$(date +%s)"
